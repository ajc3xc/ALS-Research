...
dictHead = {dfDominant.columns[0]:"Gene"}

#same header columns
for x in range(1,len(dfDominant.columns)):
    if re.search("ALS", dfDominant.columns[x]):
        dictHead.update({dfDominant.columns[x]:1})

    else:
        dictHead.update({dfDominant.columns[x]:0})
...
geneNames = list(dfDominant['Gene'].unique())
genes = len(geneNames)
...
print("Saving")
dfDominant.to_csv(r"C:\Users\13144\Desktop\Data Analysis\ALSoD2\ALSoD2_Dominant.csv")
dfRecessive.to_csv(r"C:\Users\13144\Desktop\Data Analysis\ALSoD2\ALSoD2_Recessive.csv")
print("Saved")
